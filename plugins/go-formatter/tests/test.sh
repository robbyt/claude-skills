#!/usr/bin/env bash
# Test script for go-formatter post-write hook
...
# Main test execution
echo "Running tests for go-formatter hook..."
echo
...
run_test "Hook formats Go file (Edit tool)"
TEMP_GO="$TEST_DIR/tmp/test_edit.go"
cp "$TEST_DIR/fixtures/malformed.go" "$TEMP_GO"
...
run_test "Hook ignores non-Go files"
...
if [ "$ORIGINAL_CONTENT" = "$NEW_CONTENT" ]; then
    ...
    fail_test "Hook modified non-Go file"
    rm -f "$TEMP_TXT"
    exit 1
fi

run_test "Hook handles missing file gracefully"
MISSING_FILE="$TEST_DIR/tmp/nonexistent.go"
...
# Verify gofmt formatted it (gofmt -l returns empty if file is formatted)
if [ -z "$(gofmt -l "$TEST_FILE")" ]; then
    pass_test
else
