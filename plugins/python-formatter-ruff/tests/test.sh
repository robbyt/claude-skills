#!/usr/bin/env bash
# Test script for python-formatter-ruff post-write hook
...
# Main test execution
echo "Running tests for python-formatter-ruff hook..."
echo
...
if ruff format --check "$TEMP_PY" >/dev/null 2>&1; then
    pass_test
    rm -f "$TEMP_PY"
else
...
TEST_PROMPT="This is an automated test of the python-formatter-ruff post-write hook. Please read the \
Python file at $TEST_FILE using the Read tool, then write it back unchanged using the Write tool. \
The post-write hook will automatically format the file with Ruff after the write operation. Leave \
...
# Verify Ruff formatted it
if ruff format --check "$TEST_FILE" >/dev/null 2>&1; then
    pass_test
else
