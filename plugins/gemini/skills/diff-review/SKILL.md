...
gemini "Review the code changes at .gemini-review.diff for issues" -o text 2>&1
...
4. Missing error handling" -o text 2>&1
rm .gemini-review.diff
```
...
gemini "Review the commit at .gemini-review.diff" -o text 2>&1
rm .gemini-review.diff
```
...
- Authentication/authorization issues" -o text 2>&1
rm .gemini-review.diff
```
...
- N+1 queries
- Memory leaks
- Blocking operations" -o text 2>&1
rm .gemini-review.diff
```
...
to understand the broader context." -o text 2>&1
rm .gemini-review.diff
```
...
- Add `.gemini-review.diff` to `.gitignore` to avoid accidental commits
