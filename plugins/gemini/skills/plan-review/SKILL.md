...
```bash
gemini "Review the implementation plan at [plan-file-path] and provide critique" -o text 2>&1
```
...
Read relevant source files to understand context." -o text 2>&1
```
...
Evaluate if the plan addresses the actual codebase structure." -o text 2>&1
```
...
- Data loss potential
- Rollback complexity
- Dependencies that could fail" -o text
```
...
- Are there missing steps?" -o text
```
